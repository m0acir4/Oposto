document.addEventListener('DOMContentLoaded', function() {
    ...
    const produtos = {
        "jesus-is-king": {
            ...
            images: ["/image/Blusa Jesus is king.PNG", "/image/Blusa Jesus is king 1.PNG", "/image/Blusa Jesus is king 2.PNG"],
        ...
        "yeshua-he-saves": {
            ...
            images: ["/image/Camisa Yeshua He Saves.PNG", "/image/Camisa Yeshua He Saves 1.PNG", "/image/Camisa Yeshua He Saves 2.PNG"],
            colors: ["Preto", "Branco", "Cinza", "Azul Bebê"]
        },
        "jesus-saves": {
            ...
            images: ["/image/Camisa Jesus Saves..PNG", "/image/Camisa Jesus Saves. 1.PNG", "/image/Camisa Jesus Saves. 2.PNG"],
            colors: ["Preto", "Branco", "Cinza", "Azul Bebê"]
        },
        "social-linho": {
            ...
            images: ["https://images.unsplash.com/photo-1551028719-00167b16eac5?w=500&q=80", "https://images.unsplash.com/photo-1610384104075-e0349819a56a?w=500&q=60", "https://images.unsplash.com/photo-1593030103065-b0b37f8a16af?w=500&q=60"],
            colors: ["Branco", "Azul Bebê"]
        }
    };
    
    if (document.body.classList.contains('pagina-produto')) { carregarPaginaProduto(); }
    if (document.body.classList.contains('pagina-home')) { carregarPaginaHome(); }
    ...
    function carregarPaginaHome() {
        ...
        for (const id in produtos) {
            ...
            productCard.innerHTML = `<a href="produto.html?id=${id}" class="product-link"><img src="${produto.images[0]}" alt="${produto.name}"><div class="product-card-content"><h3>${produto.name}</h3><p class="price">R$ ${produto.price.toFixed(2).replace('.', ',')}</p><span class="view-product-btn">Ver Produto</span></div></a>`;
    ...
    function carregarPaginaProduto() {
        ...
        if (!produto) { document.querySelector('main').innerHTML = '<h1>Produto não encontrado!</h1>'; return; }
    ...
    function showToast(message) { const toastContainer = document.getElementById('toast-container'); if (!toastContainer) return; const toast = document.createElement('div'); toast.classList.add('toast'); toast.textContent = message; toastContainer.appendChild(toast); setTimeout(() => toast.classList.add('show'), 100); setTimeout(() => { toast.classList.remove('show'); toast.addEventListener('transitionend', () => toast.remove()); }, 3000); }
    const openCart = () => cartModal && cartModal.classList.add('show');
    const closeCart = () => cartModal && cartModal.classList.remove('show');
    function updateCartDisplay() { if (!cartItemsContainer) return; cartItemsContainer.innerHTML = ''; let subtotal = 0; let totalItems = 0; cart.forEach(item => { const produtoInfo = produtos[item.id]; const itemSubtotal = produtoInfo.price * item.quantity; subtotal += itemSubtotal; totalItems += item.quantity; const cartItemElement = document.createElement('div'); cartItemElement.classList.add('cart-item'); cartItemElement.innerHTML = `<img src="${produtoInfo.images[0]}" alt="${produtoInfo.name}"><div class="cart-item-info"><h4>${produtoInfo.name}</h4><p>Cor: ${item.color}</p><p class="cart-item-price">R$ ${produtoInfo.price.toFixed(2).replace('.', ',')}</p><div class="cart-quantity-controls"><button class="quantity-btn" data-id="${item.id}" data-color="${item.color}" data-action="decrease">-</button><span>${item.quantity}</span><button class="quantity-btn" data-id="${item.id}" data-color="${item.color}" data-action="increase">+</button></div></div><button class="remove-item-button" data-id="${item.id}" data-color="${item.color}">&times;</button>`; cartItemsContainer.appendChild(cartItemElement); }); if (cart.length === 0) { cartItemsContainer.innerHTML = '<p>Seu carrinho está vazio.</p>'; } cartCounter.textContent = totalItems; cartSubtotal.textContent = `R$ ${subtotal.toFixed(2).replace('.', ',')}`; localStorage.setItem('cart', JSON.stringify(cart)); }
    function addToCart(productId, color, quantity) { const cartItemId = `${productId}-${color}`; const existingItem = cart.find(item => `${item.id}-${item.color}` === cartItemId); if (existingItem) { existingItem.quantity += quantity; } else { cart.push({ id: productId, color: color, quantity: quantity }); } updateCartDisplay(); }
    function handleQuantityChange(productId, color, action) { const cartItemId = `${productId}-${color}`; const itemInCart = cart.find(item => `${item.id}-${item.color}` === cartItemId); if (!itemInCart) return; if (action === 'increase') { itemInCart.quantity++; } else if (action === 'decrease') { itemInCart.quantity--; if (itemInCart.quantity <= 0) { removeFromCart(productId, color); return; } } updateCartDisplay(); }
    function removeFromCart(productId, color) { const cartItemId = `${productId}-${color}`; cart = cart.filter(item => `${item.id}-${item.color}` !== cartItemId); updateCartDisplay(); }
